...
# ---------- Page 2: Exploratory Analysis ----------
    ...
    if st.session_state.data is None:
        ...
        if st.session_state.eda_report:
            ...
            viz_tab1, viz_tab2, viz_tab3 = st.tabs(["Distributions", "Correlations", "Categorical Analysis"])
            
            with viz_tab1:
                if numeric_cols:
                    col_to_plot = st.selectbox("Select column for distribution", numeric_cols)
                    fig, ax = plt.subplots(1, 2, figsize=(12, 4))
                    ax[0].hist(df[col_to_plot].dropna(), bins=30, edgecolor='black')
                    ax[0].set_title(f'Histogram of {col_to_plot}')
                    ax[0].set_xlabel(col_to_plot)
                    ax[0].set_ylabel('Frequency')
                    
                    df[col_to_plot].dropna().plot(kind='box', ax=ax[1])
    ...
    if not st.session_state.training_complete:
        ...
        if problem_type == "Clustering":
            ...
            if isinstance(st.session_state.model, KMeans):
                ...
                uploaded_pred = st.file_uploader("Upload new data for clustering (CSV)", type=['csv'])
                if uploaded_pred is not None:
                    new_data = pd.read_csv(uploaded_pred)
                    st.write("Uploaded data preview:", new_data.head())
                    
                    # Must apply same preprocessing as during training
                    # For simplicity, we assume user knows to upload compatible data.
                    # In a full app, you'd store the scaler/encoder.
                    st.warning("This is a simplified demo. In practice, you need to apply the same preprocessing (scaling, encoding) as used during training.")
            else:
            ...
            if prediction_method == "📤 Upload New Data":
                uploaded_pred = st.file_uploader("Upload new data for prediction (CSV)", type=['csv'])
                if uploaded_pred is not None:
                    new_data = pd.read_csv(uploaded_pred)
                    st.write("Uploaded data preview:", new_data.head())
                    ...
                    if st.button("Make Predictions on Uploaded Data"):
                        with st.spinner("Predicting..."):
                            # Preprocess similarly to training (simplified)
                            # In a real app, you'd store the preprocessing steps.
                            X_new = new_data
                            if st.session_state.problem_type in ["Classification", "Regression"]:
                                # Use the TPOT pipeline (it includes preprocessing)
                ...
                if st.session_state.data is not None:
                    feature_cols = [c for c in st.session_state.data.columns if c != st.session_state.target_column]
                    input_data = {}
                    ...
                    st.warning("No reference data available. Please upload training data first.")
